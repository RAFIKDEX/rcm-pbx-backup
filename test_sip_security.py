...
class TestSipSecurity(unittest.TestCase):
    ...
    def test_failed_unban_retry(self, mock_helper):
        # Add test entry
        self.c.execute("INSERT INTO sip_security_blacklist (value, type, status, expires_at) VALUES ('1.1.1.1', 'ip', 'active', datetime('now', '-1 minutes'))")
        ...
        # Ensure single audit event
        self.c.execute("SELECT count(*) FROM sip_security_events WHERE source_ip = '1.1.1.1' AND event_type = 'Automatic Expiry'")
    ...
    def test_regex_ipv6_and_injection(self):
        ...
        # Ensures that the regex properly matches IPv6. Note: the regex matches the line but fail2ban internals validate IP extraction.
        # But wait, fail2ban-regex output contains match counts.
    ...
    def test_duplicate_reconciliation_locking(self):
        ...
        try:
            # Script should exit 0 immediately
