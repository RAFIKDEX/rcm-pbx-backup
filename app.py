...
def check_auth():
    ...
    open_endpoints = ['login', 'static', 'global_favicon', 'global_logo_png', 'forgot_password', 'forgot_password_verify', 'forgot_password_reset', 'extensions_live_status', 'dexterphone_view']
...
def api_extensions_status():
    ...
    for e in exts:
        ...
        webrtc_ext = f"{ext_num}_webrtc"
        webrtc_contacts = contacts_map.get(webrtc_ext, [])
        webrtc_reg_count = len(webrtc_contacts)
        webrtc_ep_state = endpoint_states.get(webrtc_ext, "unavailable")
        ...
        exts_status.append({
            ...
            "webrtc_reg_count": webrtc_reg_count,
...
def extensions_info(ext):
    ...
    # Also fetch WebRTC endpoint contacts
    ...
    for r in page_rows:
        ...
        if not recording:
            recording = db.find_recording_for_cdr(
                ...
                extra_parties=(
                    [x.replace("_webrtc", "") for x in re.findall(r"(?:PJSIP|SIP)/([0-9]+(?:_webrtc)?)", r.get("channel") or "")]
                    + [x.replace("_webrtc", "") for x in re.findall(r"(?:PJSIP|SIP)/([0-9]+(?:_webrtc)?)", r.get("dstchannel") or "")]
                ),
            )
        if recording:
...
def play_recording(filename):
    ...
    is_admin = user_role in ['admin', 'super_admin'] or session.get('user_type') == 'super_admin'
    has_scope = (
        _recording_scope_allows_filename('call_records', filename) or
        _recording_scope_allows_filename('cdr_reports', filename)
    )
    ...
    # Fallback to local (with recursive search)
    base_folder = "/var/spool/asterisk/monitor"
    ...
    try:
        for root, _, files in os.walk(base_folder):
            if clean_name in files:
                target_file = os.path.join(root, clean_name)
                if os.path.isfile(target_file):
                    from flask import send_file
                    return send_file(target_file, mimetype='audio/wav')
    except Exception as e:
        print(f"Error searching monitor subdirectories: {e}")
...
def api_call_events(callid):
    ...
    conn = rcm_queue_db.get_db_connection()
    call_row = conn.execute("SELECT wait_time, talk_time, hold_time FROM queue_calls WHERE uniqueid = ?", (callid,)).fetchone()
    conn.close()
...
def _cdr_find_related_transfer(cdr_row):
    ...
    try:
        ...
        context_match = re.search(r"(?:^|-)from-internal-(\d+)(?:_webrtc)?$", context, re.IGNORECASE)
